...
pub static FUNCTION_STORE: Lazy<Mutex<HashMap<String, (usize, Function)>>> = Lazy::new(|| Default::default());
...
pub use RustCallError::Error as Error;
...
fn get_file(path: &str) -> Option<(ContentType, String)> {
    if path == SERVE_PATH {
        return Some((ContentType::JavaScript, include_str!("..\\dolphine.js").to_string()))
    }
    unsafe {
...
async fn accept_connection(stream: TcpStream) {
    ...
    for (k, (arg_length, _ /* function */)) in functions.iter() {
        ...
        write.send(Message::Text(serde_json::to_string(&d).unwrap())).await.unwrap();
    }
    ...
    _ = read.try_filter(|msg| future::ready(msg.is_text()))
...
pub fn start_websocket_thread() {
    let _websocket_thread = tokio::task::spawn(
        async {
            let try_socket = TcpListener::bind(&format!("{}:{}", WEBSOCKET_ADDR, WEBSOCKET_PORT)).await;
            let listener = try_socket.expect("Failed to bind");
            println!("Listening on: {}", format!("{}:{}", WEBSOCKET_ADDR, WEBSOCKET_PORT));
            let threads = Arc::new(Mutex::new(Vec::new()));
            let threads_clone = threads.clone();
            let socket_thread = tokio::task::spawn(
                async move {
...
pub fn start_rocket_thread() {
    let _rocket_thread = tokio::task::spawn(
        async {
